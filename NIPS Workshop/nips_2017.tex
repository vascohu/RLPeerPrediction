...
%All the aforementioned peer prediction mechanisms focus on a one-shot interaction with agents\comt{For this one, we do not get into further details how this makes difference, we maybe we should just not mention it here} .
 One critical assumption in those mechanisms is an explicitly-known worker model which includes workers' incentives. Furthermore it is also assumed workers are fully rational, following utility-maximizing strategy. Unfortunately, neither is true in practice. Firstly, workers' incentives %, or cost,%
