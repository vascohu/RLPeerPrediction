...
However, existing mechanisms assume all workers can figure out and will only follow the utility-maximizing strategy, which is impractical.
Also, they only exploit a tiny part of workers' labels.
Thus, in this paper, we firstly develop a Bayesian inference algorithm to estimate workers' states by fully exploiting all the collected labels.
Then, we propose a reinforcement learning algorithm to mine the connection between workers' state changes and the incentives.
...
One of our core contributions is to theoretically prove that our mechanism correctly incentivizes workers to report high-quality labels.
