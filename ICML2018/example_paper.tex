...
\label{vot}
\begin{split}
\tilde{\sigma}^t = \frac{1}{M}{\sum}_{j=1}^{M}|\tilde{\sigma}_j^{t}|
\end{split}
\end{equation}
...
\label{utility}
\begin{split}
U = M\cdot F(\tilde{\sigma}^t) - \sum_{i=1}^{N}R^t(i)
\end{split}
\end{equation}
...
\begin{proposition}
\label{RMNE}
For our reinforcement mechanism, reporting truthfully and exerting high efforts is always a Nash equilibrium for all workers when
$$N \min_{a,b\in\mathcal{A}}(a-b) p_H\geq (1-\rho)^{-1}K_u f(p_H).$$
where $\min_{a,b\in\mathcal{A}}(a-b)$ denotes the minimal gap between any two available values of the scaling factor.
\begin{proof}
...
In this case, $\tilde{p}_i\rightarrow p_i$. Thus, the log-ratio computed in Equation~\ref{ProbRatio} satisfies
\begin{equation}
\label{ProbRatioApp}
\tilde{\sigma}_j\approx \log\left(\frac{\tau_{1}}{\tau_{2}}\lambda_i^{\delta_{ij1}-\delta_{ij2}}{\prod}_{k\neq i}\lambda_H^{\delta_{kj1}-\delta_{kj2}}\right)
\end{equation}
Besides, if the real true label of task $j$ is $1$, to ensure $\mathbb{E}[m/M]$ to approach $0$, the log-ratio $\tilde{\sigma}_j$ must approach $+\infty$.
