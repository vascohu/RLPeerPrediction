...
Suppose, at time step $t$, worker $i$ finishes $m^{t}_i$ tasks. \com{not clear; is this the number of tasks finished up to time t, or is this the ones finished at exactly time t. It reads as the latter. but then i don't under stand why$\sum_i m^t_i \gg M$. isn't exactly M tasks will be assigned?? }For any given time $t$, all $m^{t}_i$s as a whole must satisfy $\sum_i m^t_i \gg M$. Then the payment for worker $i$ at time $t$ is
\begin{equation}
...
In this subsection, we formally introduce our reinforcement learning (RL) algorithm, which adjusts the incentive scaling level at each time step $t$ ($a^t$ in Equation \ref{equation:payment}). Stepping back and viewing it under the big picture, the reinforcement learning serves as the glue to connect each other component in our framework.
...
Recall Equation~\ref{vot} shows that the estimated accuracy is calculated as an average across all $M$ tasks. 
...
\begin{equation}
Q(s,a) = \bm{k}(s,a) ^{\mathrm{T}}(\bm{K} ^{\mathrm{T}}+\bm{\sigma}^2)^{-1}\bm{H}\bm{r}
\end{equation}
...
To avoid the size of $\bm{k}$ and $\bm{K}$ to increase infinitely, we also employ the sparse approximation to discard some historical data, and the details of this approximation technique can be found in \cite{gasic2014gaussian}.
Furthermore, we use the classic $\epsilon$-greedy method to construct policy from the learned Q-function.

\com{Summarize the RL algorithm into an algorithm block. it will be much clear to reviewers.}