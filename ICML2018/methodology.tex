...
In this subsection, we formally introduce our RL algorithm, which adjusts the scaling factor at each time step $t$. Viewed under the big picture, it serves as the glue to connect each other component in our mechanism (see the edges and parmeters around RL in Figre~\ref{figure:layout}). To fully understand the technical fruit, we require the readers to be familiar with the common concepts in RL. For readers with limited knowledge, we kindly refer them to Appendix A, where we provide a tutorial on the basics. %In an RL problem, an agent interacts with an unknown environment and attempts to maximize its cumulative utility \citep{Sutton98,Szepesvari10}. 
%The environment of an RL problem is commonly formalized as a Markov Decision Process (MDP) defined as $\mathcal{M} = \langle \mathcal{S}, \mathcal{A}, \mathcal{R}, \mathcal{P}, \gamma\rangle$, where each element in the tuple denotes the state space, action space, reward function, transition probability kernel and discount rate respectively \citep{Sutton98,Szepesvari10}. The agent's goal is to learn the optimal policy $\pi(a \mid s)$.
...
>>>>>>> 3d616d085e601c96efa88bd48fdc9a372481f714
...
Under Gaussian process approximation, all the observed rewards and the corresponding $Q$ values up to the current step $t$ must satisfy
\begin{equation}
\bm{r}=\bm{H}\bm{Q}+\bm{N}
\end{equation}
