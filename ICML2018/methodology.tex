...
Suppose, at time step $t$, worker $i$ finishes $m^{t}_i$ tasks. For any given time $t$, all $m^{t}_i$s as a whole must satisfy $\sum_i m^t_i \gg M$. Then the payment for worker $i$ at time $t$ is
\begin{equation}
...
\end{equation*}
Following Bayes' theorem, we derive 
\begin{equation}
\label{PostDist}
...
$$
Since the data requester never possesses the ground truth for each task, the utility $u^t$ is not accurately observed. Also $\hat{s_t}$ cannot be accurate.% as the mapping function can not be perfect. 
...
Recall Formula~\ref{vot} shows that the estimated accuracy is calculated as an average across all $M$ tasks. 
