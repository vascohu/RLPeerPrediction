...
\begin{equation}
\textsf{effort}
P^t_i=m^t_i\cdot (a^t s^{t}_i+b)\;\; , \;\; s^{t}_i = \tilde{p}^{t}_i - 0.5
\label{equation:payment}
\end{equation}
where $\phi^{t}_i$ denotes worker $i$'s score and $\tilde{p}^{t}_i$ will be calculated by our Bayesian inference algorithm.
