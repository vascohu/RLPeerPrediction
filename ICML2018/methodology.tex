...
\label{payment}
We wish our payment rule to balance between how many tasks a worker is assigned and how good his or her labelling is. As so we design the payment for worker $i$ at time $t$ as
\begin{equation}
P^t_i=m^t_i\cdot (a^t \cdot \textsf{sc}^{t}_i+b)\;\; , \;\; \textsf{sc}^{t}_i = \tilde{\mathbb{P}}^{t}_i - 0.5
...
In this subsection, we formally introduce our reinforcement learning (RL) algorithm, which adjusts the incentive scaling level at each time step $t$ ($a^t$ in Eqn.\ref{equation:payment}). Stepping back and viewing it under the big picture, the reinforcement learning serves as the glue to connect each other component in our framework.
...
\begin{equation}
Q(s,a) = \bm{k}(s,a) ^{\mathrm{T}}(\bm{K} +\bm{\sigma}^2)^{-1}\bm{H}\bm{r}
\end{equation}
