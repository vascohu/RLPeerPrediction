...
\end{split}
\end{equation}
If we assign $m^{t}_i$ tasks to worker $i$, then worker $i$'s utility is
\begin{equation}
u_i^t=P_i^t - m^{t}_i \cdot c_H (\mathbb{P}_i^t-0.5)
\end{equation}
where $P^{t}_{i}$ denotes our payment for worker $i$ at step $t$.
