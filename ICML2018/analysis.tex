...
Recall that, Eqn.(\ref{equaton:score}) and (\ref{equation:p_infer}) together show that $\sum_j \textsf{sc}_i(j)\approx M\cdot \tilde{\mathbb{P}}_i$ holds when $M\gg 1$. Then, when $a_t>c_H/(\mathbb{P}_H-0.5)$, if $\tilde{\mathbb{P}}^t_i\approx \mathbb{P}^t_i$, worker $i$'s utility-maximizing strategy will be reporting truthfully and exerting high efforts.\footnote{$u_i^t\approx  M\cdot a_t\cdot (\tilde{\mathbb{P}}_i-0.5) + M\cdot b- M \cdot c_H\cdot \textsf{eft}^{t}_i$. See Appendix~F in Supplementary for more detailed discussion. 
